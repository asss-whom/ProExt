...
pub fn get_aimbot_yaw_pitch(
    ...
    let pos = Vector3 {
        x: aim_pos.x - camera_pos.x,
        y: aim_pos.y - camera_pos.y,
        z: aim_pos.z - camera_pos.z,
    };
...
pub fn run_aimbot(
    ...
    let base_smooth = 1.0;
    let smooth_offset = if config.smooth_offset == 0.0 {
        0.0
    } else {
    ...
    if (*LAST_MOVED.lock().unwrap()).elapsed() < ProgramConfig::CheatDelays::Aimbot
        || !game_view.world_to_screen(aim_pos, &mut screen_pos, window_info)
        || ((screen_center_x - screen_pos.x).abs() <= 1.0
            && (screen_center_y - screen_pos.y).abs() <= 1.0)
    {
        return;
    }

    let mut target_x = if screen_pos.x > screen_center_x {
        -(screen_center_x - screen_pos.x)
    } else {
        screen_pos.x - screen_center_x
    };
    target_x /= smooth;
    target_x = if screen_pos.x > screen_center_x {
    ...
    let mut target_y = if screen_pos.y > screen_center_y {
        -(screen_center_y - screen_pos.y)
    } else {
        screen_pos.y - screen_center_y
    };
    target_y /= smooth;
    target_y = if screen_pos.y > screen_center_y {
    ...
    if smooth != base_smooth {
        ...
        target_x = if target_x.abs() < base_smooth {
            if target_x > 0.0 {
                base_smooth
            } else {
                -base_smooth
        ...
        target_y = if target_y.abs() < base_smooth {
            if target_y > 0.0 {
                base_smooth
            } else {
                -base_smooth
...
pub fn aimbot_check(
    ...
    window_width: i32,
    window_height: i32,
    aim_pos: &mut Option<Vector3<f32>>,
    max_aim_distance: &mut f32,
    entity_address: &mut Option<u64>,
    address: u64,
    enemy_visible: bool,
    in_air: bool,
    distance: u32,
...
pub fn render_fov_circle(
    ui: &mut Ui,
    window_width: i32,
    window_height: i32,
    fov: i32,
    aimbot_info: Option<f32>,
    config: AimbotConfig,
) {
    let color = {
    ...
    let center_point: Vector2<f32> = Vector2 {
        x: window_width as f32 / 2.0,
        y: window_height as f32 / 2.0,
    };
    let radius = (config.fov as f32 / 180.0 * PI / 2.0).tan()
        / (fov as f32 / 180.0 * PI / 2.0).tan()
        * window_width as f32;
    ...
    if config.fov_circle_outline_enabled {
        ui.get_background_draw_list()
            .add_circle(
                center_point,
                radius,
                color_with_masked_alpha(color, 0xFF000000),
            )
            .thickness(config.fov_circle_thickness + 1.0)
            .build();
    }

    ui.get_background_draw_list()
        .add_circle(center_point, radius, color_u32_to_f32(color))
        .thickness(config.fov_circle_thickness)
        .build();
}
