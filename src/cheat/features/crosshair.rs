...
pub fn render_crosshair(
    ...
    let offset_1 = Vector2 {
        x: config.dot_size as f32,
        y: config.dot_size as f32,
    };
    let offset_2 = Vector2 {
        x: offset_1.x + 1.0,
        y: offset_1.y + 1.0,
    };
    ...
    if config.outline_enabled {
        if config.dot_enabled {
            ui.get_background_draw_list()
                .add_rect(
                    ...
                    ImColor32::from(color_with_masked_alpha(color, 0xFF000000)),
                )
                .filled(true)
                .build();
        }
        ...
        if config.lines_enabled {
            ui.get_background_draw_list()
                .add_rect(
                    Vector2 {
                        x: pos.x - (outline_gap + border_width + config.lines_width as f32),
                        y: pos.y - thickness,
                    },
                    Vector2 {
                        x: pos.x - outline_gap,
                        y: pos.y + 1.0 + thickness,
                    },
                    ImColor32::from(color_with_masked_alpha(color, 0xFF000000)),
                )
                .filled(true)
            ...
            ui.get_background_draw_list()
                .add_rect(
                    Vector2 {
                        x: pos.x + (outline_gap + dot_size),
                        y: pos.y - thickness,
                    },
                    Vector2 {
                        x: pos.x
                            + (outline_gap + dot_size + border_width + config.lines_width as f32),
                        y: pos.y + 1.0 + thickness,
                    },
                    ImColor32::from(color_with_masked_alpha(color, 0xFF000000)),
                )
            ...
            ui.get_background_draw_list()
                .add_rect(
                    Vector2 {
                        x: pos.x - thickness,
                        y: pos.y - (outline_gap + border_width + config.lines_height as f32),
                    },
                    Vector2 {
                        x: pos.x + 1.0 + thickness,
                        y: pos.y - outline_gap,
                    },
                    ImColor32::from(color_with_masked_alpha(color, 0xFF000000)),
                )
            ...
            ui.get_background_draw_list()
                .add_rect(
                    Vector2 {
                        x: pos.x - thickness,
                        y: pos.y + outline_gap + dot_size,
                    },
                    Vector2 {
                        x: pos.x + 1.0 + thickness,
                        y: pos.y
                            + (outline_gap + dot_size + border_width + config.lines_height as f32),
                    },
                    ImColor32::from(color_with_masked_alpha(color, 0xFF000000)),
                )
                .filled(true)
    ...
    if config.dot_enabled {
        ui.get_background_draw_list()
            .add_rect(
                Vector2 {
                    x: pos.x - offset_1.x + dot_size,
                    y: pos.y - offset_1.y + dot_size,
                },
                Vector2 {
                    x: pos.x + offset_1.x,
                    y: pos.y + offset_1.y,
                },
                color_u32_to_f32(color),
            )
            .filled(true)
            .build();
    }
    ...
    if config.lines_enabled {
        ui.get_background_draw_list()
            .add_rect(
                Vector2 {
                    x: pos.x - (gap + config.lines_width as f32),
                    y: pos.y - thickness + 1.0,
                },
                Vector2 {
                    x: pos.x - gap,
                    y: pos.y + thickness,
        ...
        ui.get_background_draw_list()
            .add_rect(
                Vector2 {
                    x: pos.x + gap + dot_size,
                    y: pos.y - thickness + 1.0,
                },
                Vector2 {
                    x: pos.x + (gap + dot_size + config.lines_width as f32),
                    y: pos.y + thickness,
                },
                color_u32_to_f32(color),
            )
        ...
        ui.get_background_draw_list()
            .add_rect(
                Vector2 {
                    x: pos.x - thickness + 1.0,
                    y: pos.y - (gap + config.lines_height as f32),
                },
                Vector2 {
                    x: pos.x + thickness,
                    y: pos.y - gap,
        ...
        ui.get_background_draw_list()
            .add_rect(
                Vector2 {
                    x: pos.x - thickness + 1.0,
                    y: pos.y + gap + dot_size,
                },
                Vector2 {
                    x: pos.x + thickness,
                    y: pos.y + (gap + dot_size + config.lines_height as f32),
                },
                color_u32_to_f32(color),
            )
            .filled(true)
    ...
    if config.circle_enabled {
        ui.get_background_draw_list()
            .add_circle(pos, config.circle_radius as f32, color_u32_to_f32(color))
            .build();
    }
}
