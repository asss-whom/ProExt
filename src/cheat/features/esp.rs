...
use crate::ui::functions::{
    color_u32_to_f32, color_with_alpha, color_with_masked_alpha, mix_colors, rectangle,
...
pub fn render_bones(ui: &mut Ui, bone_pos_list: [BoneJointPos; 30], config: Config) {
    let mut previous: BoneJointPos = BoneJointPos {
        pos: Vector3 {
            x: 0.0,
            y: 0.0,
            z: 0.0,
            ...
            if previous.is_visible && current.is_visible {
                if config.esp.outline {
                    ui.get_background_draw_list()
                        .add_line(
                            previous.screen_pos,
                            current.screen_pos,
                            color_with_masked_alpha(config.esp.skeleton_color, 0xFF000000),
                        )
                        .thickness(config.esp.thickness + 1.0)
                        .build();
                }
                ...
                ui.get_background_draw_list()
                    .add_line(
                        previous.screen_pos,
                        current.screen_pos,
                        color_u32_to_f32(config.esp.skeleton_color),
                    )
...
pub fn render_head(ui: &mut Ui, bone_pos_list: [BoneJointPos; 30], config: Config) {
    ...
    if config.esp.outline {
        ui.get_background_draw_list()
            .add_circle(
                center_pos,
                radius,
                color_with_masked_alpha(config.esp.head_color, 0xFF000000),
            )
            .thickness(config.esp.thickness + 1.0)
            .build();
    }
    ...
    if config.esp.head_mode == 0 {
        ui.get_background_draw_list()
            .add_circle(center_pos, radius, color_u32_to_f32(config.esp.head_color))
            .thickness(config.esp.thickness)
            .build();
    } else {
        ui.get_background_draw_list()
...
pub fn render_eye_ray(
    ui: &mut Ui,
    bone_pos_list: [BoneJointPos; 30],
    view_angle: Vector2<f32>,
    config: Config,
    view: View,
    window_info: ((i32, i32), (i32, i32)),
) {
    let line_length = (view_angle.x * PI / 180.0).cos() * 30.0;
    let head = bone_pos_list[BoneIndex::Head as usize];
    let mut pos = Vector2 { x: 0.0, y: 0.0 };

    if !view.world_to_screen(
        Vector3 {
            x: head.pos.x + (view_angle.y * PI / 180.0).cos() * line_length,
            y: head.pos.y + (view_angle.y * PI / 180.0).sin() * line_length,
            z: head.pos.z - (view_angle.x * PI / 180.0).sin() * 30.0,
        },
        &mut pos,
        window_info,
    ) {
    ...
    if config.esp.outline {
        ui.get_background_draw_list()
            .add_line(
                head.screen_pos,
                pos,
                color_with_masked_alpha(config.esp.eye_ray_color, 0xFF000000),
            )
            .thickness(config.esp.thickness + 1.0)
            .build();
    }
    ...
    ui.get_background_draw_list()
        .add_line(
            head.screen_pos,
            pos,
            color_u32_to_f32(config.esp.eye_ray_color),
        )
        .thickness(config.esp.thickness)
        .build();
}
...
pub fn get_2d_box(bone_pos_list: [BoneJointPos; 30], screen_pos: Vector2<f32>) -> Vector4<f32> {
    let head = bone_pos_list[BoneIndex::Head as usize];
    let size = Vector2 {
        x: ((screen_pos.y - head.screen_pos.y) * 1.09) * 0.6,
        y: (screen_pos.y - head.screen_pos.y) * 1.09,
    };
    let pos = Vector2 {
        x: screen_pos.x - size.x / 2.0,
...
pub fn render_snap_line(
    ...
    if config.esp.outline {
        ui.get_background_draw_list()
            .add_line(
                from_line,
                to_line,
                color_with_masked_alpha(config.esp.snap_line_color, 0xFF000000),
            )
            .thickness(config.esp.thickness + 1.0)
            .build();
    }
    ...
    ui.get_background_draw_list()
        .add_line(
            from_line,
            to_line,
            color_u32_to_f32(config.esp.snap_line_color),
        )
        .thickness(config.esp.thickness)
        .build();
}
...
pub fn render_box_bomb(ui: &mut Ui, rect: Vector4<f32>, config: Config) {
    if config.esp.outline {
        rectangle(
            ...
            color_with_masked_alpha(config.esp.bomb_color, 0xFF000000).into(),
            config.esp.thickness + 1.0,
            config.esp.rounding,
            false,
        );
    }
    ...
    rectangle(
        ...
        color_u32_to_f32(config.esp.bomb_color).into(),
        config.esp.thickness,
        config.esp.rounding,
        false,
    );
    ...
    if config.esp.filled_bomb_enabled {
        rectangle(
            ...
            color_with_alpha(config.esp.filled_bomb_color, config.esp.filled_bomb_alpha).into(),
            config.esp.thickness - 0.3,
            config.esp.rounding,
            true,
        );
    }
...
pub fn render_box(
    ...
    if config.esp.outline {
        rectangle(
            ...
            color_with_masked_alpha(config.esp.box_color, 0xFF000000).into(),
            config.esp.thickness + 1.0,
            config.esp.rounding,
            false,
        );
    ...
    rectangle(
        ...
        color_u32_to_f32(box_color).into(),
        config.esp.thickness,
        config.esp.rounding,
        false,
    );
    ...
    if config.esp.filled_box_enabled {
        ...
        rectangle_gradient(
            ...
            filled_box_color_one.into(),
            filled_box_color_two.into(),
            config.esp.thickness - 0.3,
            config.esp.rounding,
            true,
        );
...
pub fn render_weapon_name(ui: &mut Ui, weapon_name: &str, rect: Vector4<f32>, config: Config) {
    ...
    if config.esp.outline {
        stroke_text(
            ...
            Vector2 {
                x: rect.x + rect.z / 2.0,
                y: rect.y + rect.w + y_offset + 2.0,
            },
            color_u32_to_f32(config.esp.weapon_name_color).into(),
            true,
        );
    } else {
        text(
...
pub fn render_distance(ui: &mut Ui, distance: u32, rect: Vector4<f32>, config: Config) {
    ...
    if config.esp.outline {
        stroke_text(
            ...
            format!("{}m", distance),
            Vector2 {
                x: rect.x + rect.z + x_offset + 4.0,
                y: rect.y,
            },
            color_u32_to_f32(config.esp.distance_color).into(),
            false,
        );
    } else {
        text(
...
pub fn render_name(ui: &mut Ui, name: &str, rect: Vector4<f32>, config: Config) {
    ...
    if config.esp.outline {
        stroke_text(
            ...
            Vector2 {
                x: rect.x + rect.z / 2.0,
                y: rect.y - y_offset - 14.0,
            },
            color_u32_to_f32(config.esp.name_color).into(),
            true,
        );
    } else {
        text(
...
pub fn render_health_bar(ui: &mut Ui, current_health: f32, rect: Vector4<f32>, config: Config) {
    ...
    let (rect_pos, rect_size) = {
        if config.esp.bar_mode == 0 {
            ...
            (
                Vector2 {
                    x: rect.x - (height_horizontal * 1.1),
                    y: rect.y,
                },
                Vector2 {
                    x: height_horizontal,
            ...
            (
                Vector2 {
                    x: rect.x,
                    y: rect.y - (height_horizontal * 1.1),
                },
                Vector2 {
                    x: rect.z,
                    y: height_horizontal,
    ...
    let in_range = |value: f32, min: f32, max: f32| -> bool { value > min && value <= max };
    ...
    let color = {
        if in_range(proportion, 0.5, 1.0) {
            mix_colors(
                first_stage_color,
                second_stage_color,
                proportion.powf(2.5) * 3.0 - 1.0,
            )
        } else {
            mix_colors(
                second_stage_color,
                third_stage_color,
                proportion.powf(2.5) * 4.0,
            )
        }
    };
    ...
    if config.esp.bar_mode == 0 {
        ...
        ui.get_background_draw_list()
            .add_rect(
                Vector2 {
                    x: rect_pos.x,
                    y: rect_pos.y + rect_size.y - height,
                },
                Vector2 {
                    x: rect_pos.x + rect_size.x,
                    y: rect_pos.y + rect_size.y,
                },
                color,
            )
            .filled(true)
            .rounding(config.esp.rounding as f32)
            .build();
    } else {
        // Horizontal
        ui.get_background_draw_list()
...
pub fn render_armor_bar(ui: &mut Ui, armor: f32, rect: Vector4<f32>, config: Config) {
    ...
    let (rect_pos, rect_size) = {
        if config.esp.bar_mode == 0 {
            ...
            (
                Vector2 {
                    x: rect.x - (height_horizontal * multiply_factor),
                    y: rect.y,
                },
                Vector2 {
                    x: height_horizontal,
            ...
            (
                Vector2 {
                    x: rect.x,
                    y: rect.y - (height_horizontal * multiply_factor),
                },
                Vector2 {
                    x: rect.z,
                    y: height_horizontal,
...
pub fn render_ammo_bar(ui: &mut Ui, ammo: f32, max_ammo: f32, rect: Vector4<f32>, config: Config) {
    ...
    let (rect_pos, rect_size) = {
        if config.esp.bar_mode == 0 {
            ...
            (
                Vector2 {
                    x: rect.x + rect.z + (height_horizontal * 0.1),
                    y: rect.y,
                },
                Vector2 {
                    x: height_horizontal,
            ...
            (
                Vector2 {
                    x: rect.x,
                    y: rect.y + rect.w + (height_horizontal * 0.1),
                },
                Vector2 {
                    x: rect.z,
                    y: height_horizontal,
...
pub fn render_headshot_line(
    ...
    rectangle(
        ...
        Vector2 { x: 17.0, y: 3.0 },
        color_with_masked_alpha(config.esp.headshot_line_color, 0xFF000000).into(),
        1.0,
        0,
        true,
    );
    rectangle(
        ...
        Vector2 { x: 17.0, y: 3.0 },
        color_u32_to_f32(config.esp.headshot_line_color).into(),
        1.0,
        0,
        true,
    );
