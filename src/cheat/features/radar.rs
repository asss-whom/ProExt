...
pub static RADAR_RESET_POSITION: LazyLock<Arc<Mutex<Option<[f32; 2]>>>> =
...
pub fn revolve_coordinates_system(
    ...
    result_pos.x = origin_pos.x
        + (dest_pos.x - origin_pos.x) * (revolve_angle * PI / 180.0).cos()
        + (dest_pos.y - origin_pos.y) * (revolve_angle * PI / 180.0).sin();
...
pub fn render_radar(
    ...
    local_yaw: f32,
    points: Vec<(Vector3<f32>, f32, bool, bool)>,
) {
    ...
    ui.window("Radar")
        ...
        .build(|| {
            ...
            let window_pos = Vector2 {
                x: full_window_pos[0] + config.radar.range as f32,
                y: full_window_pos[1] + config.radar.range as f32,
            };
            ...
            ui.get_window_draw_list()
                .add_rect(
                    ...
                    Vector2 {
                        x: full_window_pos[0] + full_window_size[0],
                        y: full_window_pos[1] + full_window_size[1],
                    },
                    ImColor32::from_rgba_f32s(0.0, 0.0, 0.0, config.radar.alpha),
                )
                .filled(true)
                .build();
            ...
            if config.radar.crossline_enabled {
                let vertical1 = Vector2 {
                    x: window_pos.x - config.radar.range as f32,
                    y: window_pos.y,
                };
                let vertical2 = Vector2 {
                    x: window_pos.x + config.radar.range as f32,
                    y: window_pos.y,
                };

                let horizontal1 = Vector2 {
                    x: window_pos.x,
                    y: window_pos.y - config.radar.range as f32,
                };
                let horizontal2 = Vector2 {
                    x: window_pos.x,
                    y: window_pos.y + config.radar.range as f32,
                };
                ...
                ui.get_window_draw_list()
                    .add_line(vertical1, vertical2, color)
                    .build();
                ui.get_window_draw_list()
                    .add_line(horizontal1, horizontal2, color)
                    .build();
            }
            ...
            for (pos, yaw, is_visible, is_friendly) in points {
                let distance = ((local_pos.x - pos.x).powf(2.0) + (local_pos.y - pos.y).powf(2.0))
                    .sqrt()
                    / (config.radar.proportion * 100) as f32
                    * config.radar.range as f32
                    * 2.0;
                let angle = (local_yaw
                    - (pos.y - local_pos.y).atan2(pos.x - local_pos.x) * 180.0 / PI)
                    * PI
                    / 180.0;
                let point_pos = Vector2 {
                    x: window_pos.x + distance * angle.sin(),
                    y: window_pos.y - distance * angle.cos(),
                };

                if point_pos.x < window_pos.x - config.radar.range as f32
                    || point_pos.x > window_pos.x + config.radar.range as f32
                    || point_pos.y > window_pos.y + config.radar.range as f32
                    || point_pos.y < window_pos.y - config.radar.range as f32
                {
                    continue;
                }
                ...
                if config.radar.style == 0 {
                    ui.get_window_draw_list()
                        .add_circle(point_pos, circle_size, color_u32_to_f32(radar_color))
                        .filled(true)
                        .build();
                    ...
                    if config.radar.outline {
                        ui.get_window_draw_list()
                            .add_circle(
                                point_pos,
                                circle_size,
                                color_with_masked_alpha(radar_color, 0xFF000000),
                            )
                            .thickness(0.1)
                            .build();
                    }
                    ...
                    let a = revolve_coordinates_system(
                        -angle2,
                        window_pos,
                        Vector2 {
                            x: re_point.x,
                            y: re_point.y + arrow_size,
                        },
                    );
                    let b = revolve_coordinates_system(
                        -angle2,
                        window_pos,
                        Vector2 {
                            x: re_point.x - arrow_size / 1.5,
                            y: re_point.y - arrow_size / 2.0,
                        },
                    );
                    let c = revolve_coordinates_system(
                        -angle2,
                        window_pos,
                        Vector2 {
                            x: re_point.x + arrow_size / 1.5,
                    ...
                    ui.get_window_draw_list()
                        .add_polyline(vec![a, b, point_pos, c], color_u32_to_f32(radar_color))
                        .filled(true)
                        .build();
                    ...
                    let triangle_point = Vector2 {
                        x: point_pos.x
                            + (arc_arrow_size + arc_arrow_size / 3.0)
                                * (-angle2 * PI / 180.0).cos(),
                        y: point_pos.y
                            - (arc_arrow_size + arc_arrow_size / 3.0)
                                * (-angle2 * PI / 180.0).sin(),
                    };
                    let triangle_point_2 = Vector2 {
                        x: point_pos.x + arc_arrow_size * (-(angle2 - 30.0) * PI / 180.0).cos(),
                        y: point_pos.y - arc_arrow_size * (-(angle2 - 30.0) * PI / 180.0).sin(),
                    };
                    let triangle_point_3 = Vector2 {
                        x: point_pos.x + arc_arrow_size * (-(angle2 + 30.0) * PI / 180.0).cos(),
                        y: point_pos.y - arc_arrow_size * (-(angle2 + 30.0) * PI / 180.0).sin(),
                    };
                    ...
                    ui.get_window_draw_list()
                        .add_polyline(
                            vec![triangle_point, triangle_point_2, triangle_point_3],
                            ImColor32::from_rgba(220, 220, 220, 255),
                        )
                        .filled(true)
                        .build();
                }
