...
pub fn get_rcs_yaw_pitch(config: RCSConfig, rng: &mut ThreadRng) -> (f32, f32) {
    let yaw_offset = if config.yaw_offset == 0.0 {
        0.0
    } else {
...
pub fn calculate_rcs_position(
    ...
    if shots_fired > rcs_config.start_bullet {
        new_punch.x =
            ((current_punch.y - last_punch.y) * 2.0 / (pitch * config.rcs.sensitivity)) * 50.0;
        new_punch.y =
            (-(current_punch.x - last_punch.x) * 2.0 / (yaw * config.rcs.sensitivity)) * 50.0;
