...
fn main() {
    ...
    if !cfg!(debug_assertions) && ProgramConfig::Update::Enabled {
        match update_available() {
            Some(new_version) => match create_dialog(
                MessageBoxStyle::Info,
                MessageBoxButtons::YesNo,
                &format!("{} Available", new_version),
                &format!(
                    "{} {} is outdated, would you like to update?",
                    ProgramConfig::Package::Name,
                    ProgramConfig::Package::Version
                ),
            ) {
