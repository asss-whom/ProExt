...
pub fn init_gui() {
    ...
    let self_title: String = rng
        .sample_iter(&Alphanumeric)
        .take(self_title_length)
        .map(char::from)
        .collect();
    ...
    let event_loop_window: Arc<Mutex<(EventLoop<()>, Window)>> =
        Arc::new(Mutex::new(create_window(&self_title, hwnd)));
    let winit_platform_imgui_context: Arc<Mutex<(WinitPlatform, Context)>> =
        Arc::new(Mutex::new(init_imgui(&event_loop_window.lock().unwrap().1)));
