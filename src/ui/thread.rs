...
use crate::ui::main::{BG_ALPHA, EXIT, RENDER_LIST, TOGGLED, WINDOW_INFO};
    ...
    thread::spawn(move || {
        ...
        loop {
            if let Some(((x, y), (width, height))) = get_window_info(hwnd) {
                *window_info.lock().unwrap() = Some((
                    (x + window_offset, y + window_offset),
                    (width - (window_offset * 2), height - (window_offset * 2)),
                ));
            } else {
...
pub fn run_event_loop(
    event_loop_window: Arc<Mutex<(EventLoop<()>, Window)>>,
    winit_platform_imgui_context: Arc<Mutex<(WinitPlatform, Context)>>,
    hwnd: HWND,
    self_hwnd: HWND,
) {
    let window_info = WINDOW_INFO.clone();
    ...
    let (event_loop, window) = Arc::try_unwrap(event_loop_window)
        .unwrap()
        .into_inner()
        .unwrap();
    let (mut winit_platform, mut imgui_context) = Arc::try_unwrap(winit_platform_imgui_context)
        .unwrap()
        .into_inner()
        .unwrap();
        ...
        match event {
            ...
            Event::RedrawRequested(_) => {
                unsafe {
                    if toggled_value {
                        renderer
                            .gl_context()
                            .clear_color(0.0, 0.0, 0.0, *BG_ALPHA.lock().unwrap());
                    } else {
            ...
            Event::DeviceEvent {
                ...
                if let Some(keycode) = key.virtual_keycode {
                    if keycode == ProgramConfig::Keys::ToggleKey
                    ...
                    } else if keycode == ProgramConfig::Keys::ExitKey
