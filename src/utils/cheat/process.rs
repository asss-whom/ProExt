...
pub fn get_process_id(process_name: &str) -> u32 {
    ...
    let h_snapshot =
        match unsafe { CreateToolhelp32Snapshot(CREATE_TOOLHELP_SNAPSHOT_FLAGS(15), 0) } {
    ...
    unsafe {
        while Process32NextW(h_snapshot, &mut process_info).is_ok() {
            let current_name = OsString::from_wide(&process_info.szExeFile[..])
                .into_string()
                .unwrap()
                .replace("\u{0}", "");
...
pub fn get_process_module_handle(module_name: &str) -> u64 {
    ...
    unsafe {
        while Module32NextW(h_snapshot, &mut module_info).is_ok() {
            let current_name = OsString::from_wide(&module_info.szModule[..])
                .into_string()
                .unwrap()
                .replace("\u{0}", "");
