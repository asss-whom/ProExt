...
pub fn add_imgui_font(
    fonts: &mut Vec<FontSource>,
    font_path: &str,
    font_size: f32,
    range: FontGlyphRanges,
) {
...
pub fn init_imgui(window: &Window) -> (WinitPlatform, Context) {
    ...
    add_imgui_font(
        &mut fonts,
        ProgramConfig::Imgui::FontPaths::Chinese,
        font_size,
        FontGlyphRanges::chinese_full(),
    );
    add_imgui_font(
        &mut fonts,
        ProgramConfig::Imgui::FontPaths::Cryillic,
        font_size,
        FontGlyphRanges::cyrillic(),
    );
    add_imgui_font(
        &mut fonts,
        ProgramConfig::Imgui::FontPaths::Arabic,
        font_size,
        FontGlyphRanges::from_slice(&[0x600, 0x6FF, 0]),
    );
